...
public class MovementP1 : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Movement binding
        controls.MovementP1.Move.performed += ctx => moveInput = ctx.ReadValue<Vector2>();
        controls.MovementP1.Move.canceled += ctx => moveInput = ctx.ReadValue<Vector2>();

        // Jump binding
        controls.MovementP1.Jump.performed += ctx =>
        {
            if (isGrounded)
            {
    ...
    private void Attack()
    {
        ...
        {
            var health = col.GetComponent<P2Health>();
            if (health != null)
                health.health -= 10;
        }
    }
